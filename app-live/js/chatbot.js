...
(function () {
  ...
  async function q(key, table, cols, builder) {
    ...
    let query = window.sb.from(table).select(cols);
  ...
  const data = {
    proyectos: () => q('proyectos', 'proyectos',
      'id,nombre,estado,supervisor,asistente,zona,categoria,departamento,provincia,distrito,venta,facturado,por_facturar,valorizacion_pct,fecha_ini_contractual,fecha_fin_contractual,fecha_fin_real,activo,cod_proyecto',
  ...
  const tools = {
    async resumen() {
      ...
      const cfVenc = projs.filter(p => {
        ...
        return d != null && d < 0 && (Number(p.valorizacion_pct) || 0) < 99;
      });
      return { totalActivos: projs.length, enProgreso: enProg.length, sinReporte, cartera, facturado, cfVenc, last };
    },
    async sinReporte() {
      const [projs, reps] = await Promise.all([data.proyectos(), data.reportes()]);
      const today = todayISO();
  ...
  async function answer(raw) {
    ...
    if (has(n, 'cf vencid', 'vencidas', 'vencido', 'fin contractual', 'contractual pasad', 'pasaron de fecha'))
      return fmtList('CF vencidas (sin valorización completa)', await tools.cfVencidas(),
        p => `${esc(p.nombre)} · <span class="cb-bad">${Math.abs(daysUntil(p.fecha_fin_contractual))}d vencido</span> · val ${pct(p.valorizacion_pct)}`, 'calendar-x-2');
    ...
    if (has(n, 'valoriz', 'facturac', 'facturado', 'por cobrar', 'por facturar', 'cobranza', 'cartera', 'ingresos', 'venta total'))
      return fmtValorizacion(await tools.valorizacion());

    // Servicios / portafolio
    if (has(n, 'servicio', 'portafolio', 'contratado', 'licitacion'))
      return fmtServicios(await tools.servicios());

    // Contratistas
    if (has(n, 'contratista', 'contratas', 'mejor empresa', 'ranking de contratista', 'evaluacion de contratista'))
      return fmtRankCon(await tools.contratistas(8));

    // Supervisores (ranking)
    if (has(n, 'ranking de supervisor', 'mejor supervisor', 'evaluacion de supervisor', 'puntaje supervisor', 'desempeno supervisor'))
      return fmtRankSup(await tools.supervisores(8));

    // Personal / dotación
    if (has(n, 'personal', 'dotacion', 'cuanta gente', 'cuantos trabajadores', 'mano de obra', 'equipos de trabajo'))
    ...
    // Fallback
    return { html: `No identifiqué la consulta. Prueba con algo como:<br>${suggestionChips(['Resumen del día', 'Proyectos sin reporte', 'CF vencidas', 'Valorización', 'Servicios'])}<div class="cb-hint">o escribe <b>ayuda</b> para ver todo lo que puedo responder.</div>` };
  }
  ...
  function helpMsg() {
    return {
      html: `Soy el <b>Asistente PMO</b>. Consulto la data en vivo. Pregúntame por:
      <ul class="cb-ul">
        ...
        <li><b>Supervisor X</b> — sus proyectos · <b>ranking</b> de contratistas/supervisores</li>
        <li><b>Personal</b> / dotación por supervisor</li>
      </ul>
      ${suggestionChips(['Resumen del día', 'Sin reporte', 'CF vencidas', 'Valorización', 'Ranking contratistas'])}`
    };
  }
  ...
  function fmtResumen(r) {
    ...
    let html = kpiRow([
      ...
      { l: 'CF vencidas', v: r.cfVenc.length, cls: r.cfVenc.length ? 'cb-bad' : 'cb-ok' },
    ]);
    html += `<div class="cb-line"><b>Cartera:</b> ${money(r.cartera)} · <b>Facturado:</b> ${money(r.facturado)} <span class="cb-dim">(${pct(fpct)})</span></div>`;
  ...
  function fmtProyecto(r) {
    ...
    html += `<div class="cb-kv"><b>Venta:</b> ${money(p.venta)} · <b>Facturado:</b> ${money(p.facturado)} <span class="cb-dim">(${pct(p.venta ? (p.facturado / p.venta * 100) : 0)})</span></div>`;
  ...
  function fmtSupervisor(r) {
    ...
    html += kpiRow([{ l: 'Proyectos', v: list.length }, { l: 'Cartera', v: money(cartera) }]);
  ...
  function build() {
    ...
    fab.id = 'cb-fab';
    fab.setAttribute('aria-label', 'Asistente PMO');
    ...
    panel.innerHTML = `
      <div class="cb-head">
        ...
        <div><h4>Asistente PMO</h4><p>Datos en vivo · FrioPacking</p></div>
  ...
  function greet() {
    addBot({ html: `¡Hola! 👋 Soy tu <b>Asistente PMO</b>. Consulto la información en vivo de proyectos, reportes, valorización y servicios.<br>¿Qué necesitas saber?${suggestionChips(['Resumen del día', 'Sin reporte', 'CF vencidas', 'Valorización'])}` });
  }
